  ...
  <q-page>
    <div class="page-wrap">
      <div class="row items-center q-mb-lg">
        ...
        <q-space />
        <q-btn push color="primary" icon="add_business" label="Kurum Ekle" no-caps @click="dialogOpen = true" />
      </div>

      <q-banner rounded class="surface-soft q-mb-lg">
        Kurum adları işlem kayıtlarına standart biçimde bağlanır. Eski işlemlerdeki <strong>platform</strong>
        ...
        <q-list separator>
          <q-item v-for="item in institutions.institutions" :key="item.id" class="q-py-md">
            <q-item-section avatar>
              <q-avatar color="grey-2" text-color="primary" :icon="item.institution_type === 'BANK' ? 'account_balance' : 'currency_exchange'" />
            </q-item-section>
            <q-item-section>
    ...
    <q-dialog v-model="dialogOpen">
      <q-card style="width: min(94vw, 620px)">
        <q-card-section>
          ...
          <div class="text-caption text-grey-7">Kurum sözlüğe ve seçili portföy hesabına birlikte eklenir.</div>
        </q-card-section>
        <q-separator />
        <q-card-section class="row q-col-gutter-md">
          <div class="col-12"><q-input v-model.trim="draft.name" outlined label="Kurum Adı" /></div>
          <div class="col-12 col-sm-6">
            <AppPopupSelect v-model="draft.institutionType" :options="INSTITUTION_TYPES" label="Kurum Türü" :searchable="false" />
          </div>
          <div class="col-12 col-sm-6"><q-input v-model.trim="draft.countryCode" outlined maxlength="2" label="Ülke Kodu" /></div>
          <div class="col-12"><q-input v-model.trim="draft.website" outlined label="Web Sitesi" /></div>
          <div class="col-12"><q-input v-model.trim="draft.note" outlined type="textarea" autogrow label="Not" /></div>
        </q-card-section>
        <q-card-actions align="right" class="popup-action-footer q-pa-md">
          ...
          <q-btn push color="primary" icon="save" label="Kaydet" :loading="saving" no-caps @click="save" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
onMounted(async () => {
  try { await institutions.sync() }
  catch (error) { $q.notify({ type: 'negative', message: error instanceof Error ? error.message : 'Kurumlar alınamadı.' }) }
})

function isMapped(institutionId) {
  return institutions.mappings.some(
    (item) => item.account_id === portfolio.selectedAccountId && item.institution_id === institutionId && item.is_active !== false,
  )
}

async function setMapped(institutionId, active) {
  try { await institutions.setAccountMappingActive(portfolio.selectedAccountId, institutionId, active) }
  catch (error) { $q.notify({ type: 'negative', message: error instanceof Error ? error.message : 'Eşleştirme güncellenemedi.' }) }
}
...
async function save() {
  ...
  try {
    ...
    $q.notify({ type: 'negative', message: error instanceof Error ? error.message : 'Kurum kaydedilemedi.' })
