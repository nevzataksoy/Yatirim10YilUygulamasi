<template>
  <q-page>
    <div class="page-wrap notification-page">
      ...
      <q-tabs
        ...
        mobile-arrows
        outside-arrows
        class="q-mb-md notification-tabs"
      >
      ...
      <q-tab-panels v-model="tab" animated class="bg-transparent">
        ...
        <q-tab-panel name="devices" class="q-pa-none">
          <q-card flat class="section-card">
            <q-list separator>
              <q-item
                v-for="device in notifications.devices"
                :key="device.id"
                class="q-py-md notification-device-row"
                :class="{ 'notification-device-row--current': isCurrentDevice(device) }"
              >
                <q-item-section avatar>
                  <q-avatar icon="smartphone" color="grey-2" text-color="primary" />
                </q-item-section>
              ...
              <q-item v-if="!notifications.devices.length">
                <q-item-section class="text-grey-7">Kayıtlı cihaz yok.</q-item-section>
              </q-item>
            </q-list>
          </q-card>
        </q-tab-panel>
          ...
          <q-card flat class="section-card">
            <q-list separator>
              ...
              <q-item v-if="!notifications.templates.length">
                <q-item-section class="text-grey-7">Henüz otomasyon şablonu yok.</q-item-section>
              </q-item>
            </q-list>
          </q-card>
        </q-tab-panel>
          ...
          <q-card flat class="section-card">
            <q-list separator>
              <q-item v-for="log in notifications.logs" :key="log.id" class="q-py-md">
                <q-item-section avatar>
                  <q-avatar
                    :icon="log.status === 'SENT' ? 'check_circle' : 'error'"
                    :color="log.status === 'SENT' ? 'green-1' : 'red-1'"
                    :text-color="log.status === 'SENT' ? 'positive' : 'negative'"
                  />
                </q-item-section>
                <q-item-section class="min-width-0">
    ...
    <q-dialog v-model="templateDialog">
      <q-card class="notification-template-dialog">
        <q-card-section>
...
const provider = reactive({
  ...
  firebase_project_id: '',
  sender_id: '',
  android_package_name: 'tr.rosayazilim.yatirimdashboard',
  web_vapid_key: '',
  note: '',
})
...
function eventLabel(value) {
  return NOTIFICATION_EVENT_TYPES.find((item) => item.value === value)?.label || value
}
...
function removeDevice(device) {
  $q.dialog({
    ...
    message: `${device.device_name || 'Bu cihaz'} bildirim kayıtlarından silinsin mi?`,
    cancel: { label: 'Vazgeç', flat: true },
    ok: { label: 'Sil', color: 'negative', push: true },
    persistent: true,
  }).onOk(async () => {
    try {
...
async function saveTemplate() {
  try {
    await notifications.saveTemplate(draft)
...
.notification-device-row--current {
  background: rgba(15, 118, 110, 0.06);
}

@media (max-width: 599px) {
  .notification-tabs :deep(.q-tab) {
    min-width: 92px;
  }
  ...
  .notification-primary-actions .q-btn {
    width: 100%;
    margin: 0;
  }
  ...
  .notification-device-row :deep(.q-item__section--side) {
    padding-left: 6px;
  }
}
</style>
