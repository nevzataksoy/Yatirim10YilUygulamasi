...
<div class="page-subtitle q-mt-xs">Portföye dışarıdan nakit ekle veya dışarı nakit çıkar.</div>
</div>
...
<div class="col-12 col-sm-6">
<FinancialInstitutionSelect v-model="form.platform" label="Banka / Borsa / Aracı Kurum" dialog-title="Sermaye Hareketi Kurumunu Seç" />
</div>
<div class="col-12">
...
<div class="text-weight-bold">{{ form.kind === 'CASH_IN' ? 'Giriş' : 'Çıkış' }}</div>
</div>
<div class="col-6 col-sm-3">
...
no-caps
/>
<q-btn push color="primary" type="submit" icon="fact_check" label="Özeti Göster" no-caps />
</div>
</q-card-section>
</q-card>
</q-form>
...
<q-item-section side>{{ form.kind === 'CASH_IN' ? 'Sermaye Girişi' : 'Sermaye Çıkışı' }}</q-item-section>
</q-item>
<q-item>
<q-item-section>Miktar</q-item-section>
<q-item-section side :class="form.kind === 'CASH_IN' ? 'amount-positive' : 'amount-negative'">
...
availableCash.value + (form.kind === 'CASH_IN' ? Number(form.quantity || 0) : -Number(form.quantity || 0)),
)
...
$q.notify({ type: 'negative', message: error instanceof Error ? error.message : 'Kayıt başarısız.' })
} finally {
saving.value = false
}
}
