          ...
          >
            <template #append>
              <q-btn flat dense round icon="auto_fix_high" @click="fillMarketPairRate"
                ><q-tooltip>Güncel çapraz pariteyi doldur</q-tooltip></q-btn
              >
            </template>
          </q-input>
        </div>
          ...
          >
            <template #append>
              <q-btn
                ...
                ><q-tooltip>Otomatik hesaplanan miktara dön</q-tooltip></q-btn
              >
            </template>
          </q-input>
        </div>
    ...
    <q-card style="width: min(94vw, 700px)">
      ...
      <q-list separator>
        <q-item
          ...
          ><q-item-section side
            >{{ form.source_asset }} → {{ form.target_asset }}</q-item-section
          ></q-item
        >
        <q-item
        ...
        <q-item
          ...
          ><q-item-section side
            >{{ formatQuantity(form.fee_quantity, form.fee_asset) }}
            {{ form.fee_asset }}</q-item-section
          ></q-item
        >
        <q-item
...
function fillMarketPairRate() {
  ...
  if (sourceUsd > 0 && targetUsd > 0) form.pair_rate = sourceUsd / targetUsd
}
