...
try {
    ...
    & "$Root\.venv\Scripts\python.exe" -m PyInstaller --noconfirm --clean --onedir --contents-directory "_internal" --noupx --windowed --uac-admin --name InvestmentEngine --version-file version_info.txt --add-data "config;config" --collect-all psycopg --collect-submodules psycopg_pool --collect-submodules apscheduler --collect-all tzdata --collect-all firebase_admin --hidden-import win32timezone --hidden-import win32service --hidden-import win32serviceutil --hidden-import servicemanager --hidden-import pywintypes --hidden-import pythoncom --hidden-import cryptography --hidden-import websocket --hidden-import websocket._app run.py
    if (-not (Test-Path "$Root\dist\InvestmentEngine\InvestmentEngine.exe")) {
        throw "dist\InvestmentEngine\InvestmentEngine.exe oluşmadı."
    }
    if (-not (Test-Path "$Root\dist\InvestmentEngine\_internal")) {
        throw "dist\InvestmentEngine\_internal klasörü oluşmadı."
    }
    Write-Host "dist\InvestmentEngine\InvestmentEngine.exe oluşturuldu."
    Write-Host "dist\InvestmentEngine\_internal bağımlılıkları hazır."
}
